...
[width="90%",cols="2,6a"]
|===
^|*Recommendation {counter:rec-id}* |*/rec/sld10/content*
^|A |If `{baseResource}` is a Landing Page of an API that publishes a dataset (i.e., it includes a Collections resource at `/collections`), the document SHOULD contain one or more `NamedLayer` elements with names corresponding to collection ids, each with a `UserStyle`. 
^|B |If `{baseResource}` is a Collection, the document SHOULD contain one `NamedLayer` element with the name corresponding to the collection id with a `UserStyle`.
^|C |If `{baseResource}` is a Landing Page of an API that publishes no dataset (i.e., it includes no Collections resource at `/collections`), the document SHOULD contain one or more `NamedLayer` or `UserLayer` elements, each with a `UserStyle`.
|===
