...
^|A |The server SHALL process PATCH requests with a content type `application/merge-patch+json` to such a resource endpoint as specified by <<rfc7396,RFC 7396 (JSON Merge Patch)>>.
^|B |The content of the request SHALL be based upon the following OpenAPI 3.0 schema (the server can remove any metadata property from the schema that is managed by the server and not meant to be edited by clients):
