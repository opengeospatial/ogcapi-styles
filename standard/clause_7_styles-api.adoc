...
The elements of the schema are defined in <<t15_d011,"OGC Testbed-15: Encoding and Metadata Conceptual Model for Styles Engineering Report">>, except for the items in the `attributes` object, where each member describes a JSON Schema for an attribute.
...
The PUT operation on `{baseResource}/styles/{styleId}/metadata` replaces the metadata of the style with the id `styleId`. If no such style exists, an error is returned.

[[req_manage-styles_replace-style-md]]
[width="90%",cols="2,6a"]
|===
^|*Requirement {counter:req-id}* |*/req/manage-styles/replace-style-md*
^|> |The server SHALL accept style metadata based on the schema <<req_core_style-md-success,requirement /req/core/style-md-success, item B>> in all encodings supported by the API.
|===
...
In accordance with the SLD 1.0 standard, the root element of every SLD 1.0 stylesheet will be a `StyledLayerDescriptor` element.

The content of a stylesheet will typically depend on the `{baseResource}`:

* If `{baseResource}` is a Landing Page of an API that publishes a dataset (i.e., it includes a Collections resource at `/collections`), the document will typically contain one or more `NamedLayer` elements with names corresponding to collection ids, each with a `UserStyle`. 
* If `{baseResource}` is a Collection, the document will typically contain one `NamedLayer` element with the name corresponding to the collection id with a `UserStyle`.
* If `{baseResource}` is a Landing Page of an API that publishes no dataset (i.e., it includes no Collections resource at `/collections`), the document will typically contain one or more `NamedLayer` or `UserLayer` elements, each with a `UserStyle`.

If a `UserStyle` has a name, it will typically correspond to the style id, but the name is not relevant.

If a `UserStyle` has `FeatureTypeStyle` elements with a name, the name will typically correspond to the collection id.

If the `NamedLayer` has a `Name` and a `FeatureTypeStyle` with a `FeatureTypeName`, both should be consistent. Clients should be prepared to understand either of them, with the more specific `FeatureTypeName` taking priority (as in some cases the `Name` of the `NamedLayer` may refer to a particular symbolization, e.g. casing or center line).
...
NOTE: This is a starting point for discussion and this may be converted into requirements and recommendations. See also [Issue 24](https://github.com/opengeospatial/ogcapi-styles/issues/24).
